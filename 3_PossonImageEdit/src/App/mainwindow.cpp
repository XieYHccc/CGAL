...
void MainWindow::CreateActions()
{
	...
	action_choose_Free_ = new QAction(tr("FreeChoose"), this);
	connect(action_choose_Free_, SIGNAL(triggered()), this, SLOT(Choose_FreeShape()));
...
void MainWindow::CreateToolBars()
{
	...
	toolbar_file_->addAction(action_choose_Free_);
...
{
	QString filename = QFileDialog::getOpenFileName(this, tr("Read Image"), "../data");
	if (!filename.isEmpty())
	{
		...
		if (child->LoadFile(filename))
		{
			...
			int x = child->geometry().x();								// (x,y) is the interior coordinate of top left corner
...
void MainWindow::ChooseRect()
{
	...
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_Rectchoose();
}

void MainWindow::Choose_FreeShape()
{
	ChildWindow* window = GetChildWindow();
	if (!window)
		return;
	window->imagewidget_->set_draw_status_to_Freechoose();
}
...
void MainWindow::copy()
{
	...
	if (!window)
		return;
	if (window->imagewidget_->set_copy())
	{
		child_source_ = window;
	}
	else
	{
		QMessageBox::about(this, tr("About"), tr("please select the region to copy first"));
	}
